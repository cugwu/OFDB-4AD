...
# weights from the official github repository https://github.com/raoyongming/GFNet/tree/master?tab=readme-ov-file
model_path = {
    'gfnet_h_ti': '',
    'gfnet_h_s': '',
    'gfnet_h_b': '',
...
def _gfnet_h(arch = "gfnet_h_s",
             ...
             pretrained=True,
             device='cpu',
             **kwargs: Any,) -> GFNetPyramid:
    model = GFNetPyramid(img_size=img_size, patch_size=patch_size, embed_dim=embed_dim, depth=depth, mlp_ratio=mlp_ratio,
        norm_layer=norm_layer, drop_path_rate=drop_path_rate, init_values=init_values, **kwargs)
    if pretrained:
        state_dict = torch.load(model_path[arch], map_location=torch.device(device))
        model.load_state_dict(state_dict['model'])
    return model

def _gfnet(arch = "gfnet_s",
             img_size=224,
             patch_size=16,
             embed_dim=384,
             depth=19,
             ...
             device='cpu',
             **kwargs: Any) -> GFNet:
    model = GFNet(img_size=img_size, patch_size=patch_size, embed_dim=embed_dim, depth=depth, mlp_ratio=mlp_ratio,
        norm_layer=norm_layer, drop_path_rate=drop_path_rate, **kwargs)
    if pretrained:
        state_dict = torch.load(model_path[arch], map_location=torch.device(device))
        model.load_state_dict(state_dict['model'])
    return model
