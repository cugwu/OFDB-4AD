...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        batch, c, h, w = x.size()
        ffted = torch.fft.rfft2(x, s=(h, w), dim=(2, 3), norm='ortho')
        ...
        output = torch.fft.irfft2(ffted, s=(h, w), dim=(2, 3), norm='ortho')
...
class FFCBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        super(FFCBottleneck, self).__init__()
        width = int(planes * (base_width / 64.)) * groups
        # Both self.conv2 and self.downsample layers downsample the input when
        # stride != 1
...
class FFCResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, FFCBottleneck):
                    nn.init.constant_(m.bn3.weight, 0)
                elif isinstance(m, FFCBasicBlock):
